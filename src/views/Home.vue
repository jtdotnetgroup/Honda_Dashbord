<template>
  <div style="margin-top: 60px;">
    ...
    <van-overlay :show="isLoad">
      <div class="wrapper">
        <van-loading size="24px" type="spinner" color="#1989fa"
          >加载中({{ showPercentage }}%)...</van-loading
        >
      </div>
    </van-overlay>
    <!-- 定时刷新秒数 -->
    ...
    ></el-date-picker>
    <el-button type="primary" round @click="search" :disabled="startend == null"
      >查询</el-button
    >
    <!-- 增加高度 -->
    <div>
...
export default {
  ...
  watch: {
    ...
    count: function(newQuestion) {
      ...
      // 加载完毕开始填充Echarts
      if (newQuestion == this.MusicTypeCount * this.Hour) {
        //
        this.LoadDataE(this.$GetList(this.datalist, this.currentType));
        this.isLoad = false;
        this.checked = true;
  ...
  methods: {
    test() {
      ...
      axios.get("/static/musictype.json").then(res => {
        if (res.status === 200) {
          var data = res.data;
          _this.currentType = data.currentType;
    ...
    search() {
      ...
      for (var i = 0; i < _this.dateList.length; i++) {
        // 遍历获取设备状态
        _this.currentType.forEach(item => {
